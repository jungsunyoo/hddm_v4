...
def wiener_like_rlddm_2step(np.ndarray[long, ndim=1] x1, # 1st-stage RT
                      np.ndarray[long, ndim=1] x2, # 2nd-stage RT
                      ...
                      np.ndarray[long, ndim=1] feedback,
                      np.ndarray[long, ndim=1] split_by,
    ...
    cdef np.ndarray[long, ndim=1] x1s
    cdef np.ndarray[long, ndim=1] x2s
    cdef np.ndarray[long, ndim=1] s1s
    cdef np.ndarray[long, ndim=1] s2s    
    cdef np.ndarray[long, ndim=1] feedbacks
    ...
    cdef np.ndarray[long, ndim=2] qs_mf = np.ones((comb(nstates,2,exact=True),2))*q # first-stage MF Q-values
    cdef np.ndarray[long, ndim=2] qs_mb = np.ones((nstates, 2))*q # second-stage Q-values

    cdef np.ndarray[int, ndim=1] counter = np.zeros(comb(nstates,2,exact=True))

    cdef np.ndarray[long, ndim=1] Qmb
    ...
    cdef np.ndarray[long, ndim=2] Tm = np.array([[0.7, 0.3], [0.3, 0.7]]) # transition matrix
    ...
    for j in range(unique.shape[0]):
        ...
        for i in range(0, s_size):
            ...
            for s_ in range(comb(nstates,2)):
                for a_ in range(2):
